...
server.tool(
  ...
  async ({ image_url, video_url, caption, scheduled_publish_time, media_type }) => {
    requireAuth();
    if (!image_url && !video_url) throw new Error("Either image_url or video_url is required");
    ...
    if (media_type === "VIDEO") { body.media_type = "VIDEO"; body.video_url = video_url; }
    else if (media_type === "REELS") { body.media_type = "REELS"; body.video_url = video_url; }
